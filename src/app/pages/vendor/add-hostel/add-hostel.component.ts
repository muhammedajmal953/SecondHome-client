...
export class AddHostelComponent {
  ...
  facilities: string[] =[];
  bedTypes: string[] = ['2 share','4 share', '6 share'];
  nearbyPlaces: string[]=[];
...
console.log('clicked');
  ...
  atLeastOneCheckboxSelected(control: AbstractControl): { [key: string]: any } | null {
    const checkboxes = control as FormGroup;
    const selectedCount = Object.values(checkboxes.value).filter(value => value === true).length;
